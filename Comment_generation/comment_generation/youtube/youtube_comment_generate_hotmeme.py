...
# ── Step 3: Label 分类参数 ────────────────────────────────────────
SAMPLE_JSON_PATH  = os.path.join(BASE_DIR, "data_pre", "code_active", "youtube", "youtube_video_sample.json")
...
COMMENT_CONFIG = {
    ...
    "input_video_file":     os.path.join(
        BASE_DIR, "data_pre", "json", "youtube", "data_pre", "youtube_video_description_codex.json"
    ),

    # 输入：学习样本
    "learning_sample_file": os.path.join(
        BASE_DIR, "data_pre", "json", "youtube", "sample", "youtube_sample_codex.json"
    ),

    # 输出：生成的评论结果
    "output_comment_file":  os.path.join(
        BASE_DIR, "comment_generation", "json", "result", "youtube_output_comments.json"
    ),

    # 缓存：预计算的 embedding
    "cache_file":           os.path.join(
        BASE_DIR, "comment_generation", "cache", "youtube_cached_samples.pkl"
    ),

    # 热梗缓存文件夹（网络搜索后持久化）
    "hot_meme_folder":      os.path.join(
        BASE_DIR, "comment_generation", "hotmeme"
    ),
...
def prepare_comment_jobs(video_data: list[dict], samples: list, label_index: dict,
                         enable_meme_search: bool = True) -> list[dict]:
